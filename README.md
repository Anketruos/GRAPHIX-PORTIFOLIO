<<<<<<< HEAD
# Aster Teshome — Graphix Portfolio ✦

> Bold. Intentional. Unforgettable.

**Live site →** [graphix-portifolio.vercel.app](https://graphix-portifolio.vercel.app)

---

## Overview

Personal graphic design portfolio for Aster Teshome (Anket) — a junior graphic designer with a focus on community work, event branding, and typography. Built from scratch with no frameworks.

---

## Work Categories

- 🎨 **SheCodes AASTU** — event posters and social media graphics for the women in tech community
- ✝ **Fellowship Graphics** — program posters, scripture typography, and devotional visuals
- ☁ **AWS Student Builders** — recruitment posts, launch announcements, and community graphics for AWS Student Builders Group at AASTU
- 🖋 **Personal Projects** — experimental designs and creative side work

---

## Features

- 🌙 Dark / light mode — dark by default, preference persists across sessions
- 🖼 Lightbox viewer with keyboard navigation
- ✉ Contact form powered by [Formspree](https://formspree.io) — sends directly to inbox
- ⚡ All images in `.webp` format for fast load times
- 📱 Fully responsive with mobile navigation

---

## Stack

Plain HTML · CSS · JavaScript — no frameworks, no build tools.
Fonts via Google Fonts · Hosted on Vercel.

---

## Structure

```
GRAPHIX PORTIFOLIO/
├── index.html
├── images/          # S = SheCodes · F = Fellowship · A = AWS · P = Personal
└── pages/
    ├── style.css    # global styles + dark/light theme
    ├── script.js    # nav, theme toggle, animations
    ├── shecodes     # .html .js
    ├── fellowship   # .html .js
    ├── aws          # .html .js
    ├── personal     # .html .js
    ├── about        # .html .css .js
    └── contact      # .html .css .js
```
=======
# Aster Teshome — Graphix Portfolio ✦

> Bold. Intentional. Unforgettable.

**Live site →** [graphix-portifolio.vercel.app](https://graphix-portifolio.vercel.app)

---

## Overview

Personal graphic design portfolio for Aster Teshome (Anket) - a junior graphic designer with a focus on community work, event branding, and typography. Built from scratch with no frameworks.

---

## Work Categories

- 🎨 **SheCodes AASTU** - event posters and social media graphics for the women in tech community
- ✝ **Fellowship Graphics** - program posters, scripture typography, and devotional visuals
- 🖋 **Personal Projects** - experimental designs and creative side work

---

## Features

- 🌙 Dark / light mode - dark by default, preference persists across sessions
- 🖼 Lightbox viewer with keyboard navigation
- ✉ Contact form powered by [Formspree](https://formspree.io) - sends directly to inbox
- ⚡ All images in `.webp` format for fast load times
- 📱 Fully responsive with mobile navigation

---

## Stack

Plain HTML · CSS · JavaScript - no frameworks, no build tools.
Fonts via Google Fonts · Hosted on Vercel.

---

## Structure

```
GRAPHIX PORTIFOLIO/
├── index.html
├── images/          # S = SheCodes · F = Fellowship · P = Personal
└── pages/
    ├── style.css    # global styles + dark/light theme
    ├── script.js    # nav, theme toggle, animations
    ├── shecodes     # .html .js
    ├── fellowship   # .html .js
    ├── personal     # .html .js
    ├── about        # .html .css .js
    └── contact      # .html .css .js
```
>>>>>>> 3ead44e791d59e6b3aa65a5d4727c0b57cbc0ef4
